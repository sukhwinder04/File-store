    ...
    buttons = [
        [
            InlineKeyboardButton(
                "⚡𝗝𝗼𝗶𝗻 𝗖𝗵𝗮𝗻𝗻𝗲𝗹1⚡",
                url = "https://t.me/+GKYy820ZISE5Mjg1"),
            InlineKeyboardButton(
                "⚡𝗝𝗼𝗶𝗻 𝗖𝗵𝗮𝗻𝗻𝗲𝗹2⚡",
                url = "https://t.me/+kuBgeBOkWpMyNWQ1")
        ]
    ]
    try:
        buttons.append(
...
@Bot.on_message(filters.command('start') & filters.private & subscribed & premium)
async def start_command(client: Client, message: Message):
    if not await present_user(message.from_user.id):  # Fixed 'id' to 'message.from_user.id'
        try:
    ...
    if len(message.text) > 7:
        ...
        phdlusts = []
        for msg in messages:
            if bool(CUSTOM_CAPTION) and bool(msg.document):  # Fixed '&' to 'and'
